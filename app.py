...
# Evaluaciones dinámicas respN_i y estuN_i para N=3..17
for n in range(3, 18):
    for i in range(5):
...
DOC_FIELDS += [
    'Municipio','Estado',
    'Actividad1','Logro 1','Actividad2','Logro 2','Actividad3','Logro 3','Actividad4','Logro 4',
    'Actividad5','Logro 5','Actividad6','Logro 6','Actividad7','Logro 7','Actividad8','Logro 8',
...
# ── CRUD Documentos: lista, nueva, edición y eliminación ─────────────────────
...
@login_required
def documentos_new():
    flash("Para crear un documento, usa los botones de Solicitud, Bimestral o Final.", "info")
    return redirect(url_for('documentos_list'))

@app.route('/documentos/edit/<no_control>')
@login_required
def documentos_edit(no_control):
    # redirige a la solicitud por defecto
...
def documentos_bimestral(no_control, num):
    ...
    if request.method == 'POST':
        ...
        # 1) Bimestre: radio group "bim"
        sel_bim = request.form.get('bim')
        for i in ('1','2','3'):
            doc[f'x{i}'] = 'X' if sel_bim == i else ''

        # 2) Fechas del bimestre
        for part in ('Dia1','Mes1','Anio1','Dia2','Mes2','Anio2'):
            doc[f"{part}_{num}"] = request.form.get(f"{part}_{num}", '')

        # 3) Supervisor y actividades
        ...
        # 4) Evaluaciones: radio groups respN and estuN for N=3..17
        for n in range(3, 18):
            # clear all five slots
            for i in range(5):
                doc[f'resp{n}_{i}'] = ''
                doc[f'estu{n}_{i}'] = ''

            sel_resp = request.form.get(f'resp{n}')
            sel_estu = request.form.get(f'estu{n}')
            if sel_resp and sel_resp.isdigit():
                doc[f"resp{n}_{sel_resp}"] = 'X'
            if sel_estu and sel_estu.isdigit():
                doc[f"estu{n}_{sel_estu}"] = 'X'
