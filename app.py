...
# Rutas de archivos CSV y plantillas
...
# Sinónimos de campos (puedes extender esto)
FIELD_SYNONYMS = {
    'ap': 'Apellido_Paterno',
    'am': 'Apellido_Materno',
    'no_de_control': 'No_Control',
    'num_control': 'No_Control',
    # agrega aquí si ocupas más
}
...
# Funciones auxiliares para cargar y guardar CSV
...
def documentos_solicitud(no_control):
    ...
    if request.method=='POST':
        for f in ['Apellido_Paterno','Apellido_Materno','Nombre','Sexo','Telefono','Correo','Domicilio','Carrera','Periodo','Semestre','Creditos','Dependencia','Domicilio_Dependencia','Titular_Dependencia','Cargo_Responsable','Responsable_Proyecto','Nombre_Programa','Fecha_Inicio','Fecha_Terminacion','Actividades']:
            if not request.form.get(f):
                flash(f"Completa el campo: {f}","warning")
                return redirect(request.url)
        tps = [f for f in ['TP_Edu_Adultos','TP_Desarrollo','TP_Deportivo','TP_Cultural','TP_Civico','TP_Sustentable','TP_Salud','TP_Medio_Amb','TP_Otros'] if request.form.get(f)]
        if len(tps)!=1:
            flash("Selecciona solo un tipo de proyecto","warning")
            return redirect(request.url)
        for part in ('Dia_Solicitud','Mes_Solicitud','Anio_Solicitud'):
            if not request.form.get(part):
                flash(f"Completa el campo: {part}","warning")
                return redirect(request.url)
        if not doc:
        ...
        for f in ['TP_Edu_Adultos','TP_Desarrollo','TP_Deportivo','TP_Cultural','TP_Civico','TP_Sustentable','TP_Salud','TP_Medio_Amb','TP_Otros']:
            doc[f]='X' if f in tps else ''
        for part in ('Dia_Solicitud','Mes_Solicitud','Anio_Solicitud'):
